...
type Expectation struct {
	...
	// routed marks an order-independent Route rather than a queued Expect.
	// respondFn, when set, computes the response per call; calls counts how
	// many requests this expectation has answered.
	routed    bool
	respondFn func(r *http.Request, call int) (int, string)
	calls     int
}
