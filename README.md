...
- [`tfprovider`](./tfprovider) — OpenTofu / Terraform provider
  implementation.
- [`terraform-provider-gigahost`](./terraform-provider-gigahost) —
  standalone Go module that ships as `kradalby/terraform-provider-gigahost`
  on the OpenTofu Registry (primary) and the Terraform Registry.
...
Go 1.26 + golangci-lint + OpenTofu toolchain. No `make`, no `nix develop`
wrapper needed:
...
- **JSON v2**: every `json` import is
  [`github.com/go-json-experiment/json`](https://github.com/go-json-experiment/json)
  — the reference implementation that became `encoding/json/v2`.
