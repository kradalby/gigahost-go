...
func TestNormalizeASNImportID(t *testing.T) {
	...
	tests := []struct {
		...
		// unicode.IsDigit accepts every Nd code point; the documented
		// contract is ASCII only, so these must be rejected.
		{in: "AS٢١٢", wantErr: true},
		{in: "२१२३४५", wantErr: true},
		{in: "２１２３４５", wantErr: true},
	}
