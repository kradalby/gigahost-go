...
type gigahostProvider struct {
	...
	// httpClient, when set, is handed to the API client. Test-only: it is how
	// the in-process harness reaches an in-memory fake API. The provider
	// binary leaves it nil and the client builds its own.
...
func (p *gigahostProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	if p.httpClient != nil {
		opts = append(opts, gigahost.WithHTTPClient(p.httpClient))
	}
...
// Option configures the provider. It exists so in-process tests can point the
// real resources at a fake API on httptest's in-memory network; the provider
// binary passes none.
type Option func(*gigahostProvider)

// withHTTPClient makes the provider's API client use hc. Test-only: the poll
// loops run at their shipping cadence and are driven by a synctest clock, so
// the fake API must be reachable without touching the network.
func withHTTPClient(hc *http.Client) Option {
	return func(p *gigahostProvider) { p.httpClient = hc }
}
...
func New(version string, opts ...Option) func() provider.Provider {
	return func() provider.Provider {
		p := &gigahostProvider{version: version}
