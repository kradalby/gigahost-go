...
// whitespace) and returns the bare numeric string. Only ASCII digits count:
// unicode.IsDigit would also accept Devanagari or Arabic-Indic digits, which
// no downstream consumer can parse. Any other form is an error.
...
// unixOrNull renders a timestamp as Unix seconds, or null when the API did
// not supply one. A zero time.Time renders as -62135596800, which is not a
// date anybody wants to see in state or in a plan diff.
func unixOrNull(t time.Time) types.Int64 {
	if t.IsZero() {
