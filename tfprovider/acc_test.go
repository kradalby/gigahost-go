...
func accZoneName(t *testing.T, prefix string) string {
	...
	return fmt.Sprintf("%s%s-%06d.%s", sweepPrefix, prefix, rand.Intn(1_000_000), apex)
}
...
func testAccGigahostClient(t *testing.T) *gigahost.Client {
	...
	// Gate on TF_ACC here, not only in resource.Test's PreCheck. Callers reach
	// for a live client in the test body — to pick the cheapest server type, to
	// create an object out of band — which runs before PreCheck ever fires. A
	// plain `go test ./...` with a token in the environment would otherwise
	// talk to the real API, and one edit away from a billed deploy.
	if os.Getenv("TF_ACC") == "" {
