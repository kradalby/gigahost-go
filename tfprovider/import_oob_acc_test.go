...
func requireToken(t *testing.T) {
	...
	// These tests create real objects through the CLI *before* resource.Test
	// runs, so its own TF_ACC gate is too late to protect anything. Without
	// this check a plain `go test ./...` with a token in the environment
	// renames live servers and creates real DNS zones.
	if os.Getenv("TF_ACC") == "" {
...
func TestAccImportOOB_DNSZone(t *testing.T) {
	...
	runCLIJSON(t, &created, "dns", "zones", "create", "--type", "NATIVE", zoneName)
