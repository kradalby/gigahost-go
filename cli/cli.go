...
func Run(ctx context.Context, args []string, opts Options) int {
	...
	// ff stops parsing flags at the first positional, so `dns zones create
	// NAME --type NATIVE` would silently drop --type. Reorder first, so the
	// habit every other CLI allows works here too.
	args = hoistFlags(cmd, args)
	...
	if err := cmd.ParseAndRun(ctx, args); err != nil {
		...
		case errors.Is(err, ff.ErrNoExec):
			// A bare group command ("gigahost dns") and a typo
			// ("gigahost dns creat") arrive here alike. Both are usage
			// errors: help goes to stderr and the exit code is non-zero, so
			// `gigahost dns zones creat x || echo failed` actually fires.
			fmt.Fprintln(opts.Stderr, ffhelp.Command(cmd))
