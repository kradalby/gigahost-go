...
func newDNSRecordsCmd(c *Context, parent *ff.FlagSet, load func() error) *ff.Command {
	...
	fs.StringVar(&p.recValue, 'V', "value", "", "record value (required to delete: the API matches on value, not on RECORD_ID)")
	fs.IntVar(&p.recTTL, 0, "ttl", 3600, "record TTL")
	fs.IntVar(&p.recPrio, 0, "priority", -1, "record priority (MX; required to delete an MX record)")
...
func dnsRecordsUpdate(ctx context.Context, c *Context, load func() error, args []string, p *dnsRecordParams) error {
	...
	// The update is a whole-record replace, and --type and --ttl carry
	// defaults. Reading them unconditionally meant `--value 1.2.3.4` on an MX
	// record rewrote it as an A record, and `--ttl 600` alone blanked the
	// value. Require the full record instead of silently filling the gaps.
	if p.zoneID == "" || p.recName == "" || p.recValue == "" {
		return errors.New("--zone, --name and --value are required: update replaces the " +
			"whole record, so omitting one would reset it to the flag default")
	}
...
func dnsRecordsDelete(ctx context.Context, c *Context, load func() error, args []string, p *dnsRecordParams) error {
	...
	// --value is required as well: the API deletes every record sharing the
	// name and type unless the value narrows it to one (upstream B21).
	if p.zoneID == "" || p.recName == "" || p.recType == "" || p.recValue == "" {
		return errors.New("--zone, --name, --type and --value are required")
	}
	...
	if p.recPrio >= 0 {
		prio := p.recPrio
		deleteReq.Priority = &prio
	}

	if err := client.DNS.DeleteRecord(ctx, zid, args[0], deleteReq); err != nil {
		return err
	}
