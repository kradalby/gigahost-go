...
func (s *apiString) UnmarshalJSON(data []byte) error {
	...
	// Unquoted (a JSON number): the raw token is the value we want.
	*s = apiString(trimmed)
...
// marshalJSON is a small shim so service code doesn't need to import the
// json/v2 package directly and so that future migration to
// encoding/json/v2 in stdlib is a single-file change.
