...
| **Power state** | `gigahost_server_power` (new) | create + update | in-place (GET power/on\|off\|reboot) | Declarative desired power state, continuously reconciled; never rebuilds. |
...
|---|---|---|---|---|
| `product_id` | RequiresReplace | **RequiresReplace on `gigahost_server`**, but real changes go through `gigahost_server_upgrade` | `POST /servers/{id}/upgrade` (`pkg_id`) — same ID + IP | The deploy resource fixes the *initial* product. Post-deploy resizes are an explicit `gigahost_server_upgrade` resource, not an edit to this attribute. Do **not** drop `RequiresReplace` here: an upgrade does not change `product_id` *at deploy time*, and making it mutable would create two writers for one field. Downgrade is likely API-blocked (confirm). |
...
Relevant existing files this design builds on: `/Users/kradalby/git/gigahost-go/tfprovider/server_resource.go` (the all-RequiresReplace deploy resource to correct), `/Users/kradalby/git/gigahost-go/tfprovider/server_rdns_resource.go` and `/Users/kradalby/git/gigahost-go/tfprovider/server_name_resource.go` (the anchored-mutable-resource pattern to follow), `/Users/kradalby/git/gigahost-go/tfprovider/locks.go` (the per-entity serialization pattern), `/Users/kradalby/git/gigahost-go/tfprovider/provider.go` (registration), and the client services `/Users/kradalby/git/gigahost-go/client/{power,reinstall,upgrades,snapshots,isos,ipmi,servers}.go`.
...
- **Definitive capability matrix for the hourly KVM VPS** (probed on a fully
  ...
  | reboot (`GET/POST /reboot`) | ❌ 400 | not supported — no resource |
