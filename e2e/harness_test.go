...
func cheapestTarget(t *testing.T, c *gigahost.Client) deployTarget {
	...
	for _, tier := range cat.Tiers {
		for _, p := range tier.Products {
			...
			// VMs only. A dedicated box is far more expensive and slower to
			// provision than anything these tests need; GIGAHOST_TEST_PRODUCT
			// is the deliberate opt-out.
			if pin == "" && p.PlatformSlug() != gigahost.PlatformCloud {
				continue
			}
